...
First implementation.
  ...
  model TestBaseCircuit
    ...
    model pictures
  BuildingSystems.Fluid.Storage.ExpansionVessel exp annotation(
  ...
  model CommunitySystemOnGrid "Photovoltaic cooling system"
    ...
    Modelica.Units.SI.Energy EGrid(start = 0.0) "Integrates the electricity taken from the grid";
    Modelica.Units.SI.Energy EPVField(start = 0.0) "Integrates the electricity generated by the PV field";
    BuildingSystems.Fluid.HeatPumps.Carnot_TCon heatPump(redeclare package Medium1 = Medium1, redeclare package Medium2 = Medium2, QCon_flow_nominal = 13000, allowFlowReversal1 = false, allowFlowReversal2 = false, dp1_nominal = 1000, dp2_nominal = 1000, show_T = true) annotation(
    ...
    BuildingSystems.Fluid.Sources.Boundary_pT boundary1(redeclare package Medium = Medium2, T = 285.15, nPorts = 1, use_T_in = true) annotation(
    ...
    BuildingSystems.Fluid.Sensors.TemperatureTwoPort tempStorageOUT(redeclare package Medium = Medium1, m_flow_nominal = 0.1) annotation(
      Placement(transformation(origin = {106, -26}, extent = {{-10, -10}, {10, 10}})));
    BuildingSystems.Fluid.Storage.ExpansionVessel expRad(redeclare package Medium = Medium1, T_start = 283.15, V_start = 0.1, p_start = 1.5e5) annotation(
      Placement(transformation(origin = {190, -56}, extent = {{-10, -10}, {10, 10}})));
    ...
    Modelica.Blocks.Math.BooleanToInteger booToInt1 annotation(
      Placement(transformation(origin = {62, 42}, extent = {{0, -80}, {10, -70}})));
    BuildingSystems.Fluid.Movers.FlowControlled_m_flow pumpHP(redeclare package Medium = Medium1, energyDynamics = Modelica.Fluid.Types.Dynamics.SteadyState, inputType = BuildingSystems.Fluid.Types.InputType.Stages, m_flow_nominal = 0.4) annotation(
  ...
  equation
    ...
    connect(radiation.IrrDifHor, ambience.IrrDifHor) annotation(
      Line(points = {{-51.6, 10}, {-58, 10}, {-58, 30}, {83, 30}, {83, 27}}, color = {0, 0, 127}));
    connect(ambience.TAirRef, pvField.TAmb) annotation(
    ...
    connect(heatPump.port_b2, boundary1.ports[1]) annotation(
    ...
    connect(not11.y, booToInt1.u) annotation(
      Line(points = {{56.6, -32}, {56.6, -33}, {61, -33}}, color = {255, 0, 255}));
    connect(storage.port_HX_1_a, pumpHP.port_b) annotation(
      Line(points = {{129, -66}, {98, -66}}, color = {0, 127, 255}));
    connect(pumpHP.port_a, heatPump.port_b1) annotation(
      Line(points = {{78, -66}, {48, -66}, {48, -126}, {122, -126}}, color = {0, 127, 255}));
    ...
    connect(expRad.port_a, storage.port_a1) annotation(
      Line(points = {{190, -66}, {145, -66}, {145, -71}, {143, -71}}, color = {0, 127, 255}));
    connect(booToInt1.y, pumpHP.stage) annotation(
      Line(points = {{72.5, -33}, {88, -33}, {88, -54}}, color = {255, 127, 0}));
    connect(heatPump.P, battery.PLoad) annotation(
    ...
    connect(ambience.TAirRef, boundary1.T_in) annotation(
      Line(points = {{84, 34}, {2, 34}, {2, -162}, {202, -162}, {202, -144}, {194, -144}}, color = {0, 0, 127}));
    connect(tempStorageOUT.port_b, pumpRad.port_a) annotation(
      Line(points = {{116, -26}, {124, -26}}, color = {0, 127, 255}));
    connect(tempStorageOUT.port_a, storage.port_b1) annotation(
      Line(points = {{96, -26}, {90, -26}, {90, -46}, {156, -46}, {156, -52}, {144, -52}}, color = {0, 127, 255}));
    annotation(
      experiment(StartTime = 0, StopTime = 31536000),
      __Dymola_Commands(file = "modelica://BuildingSystems/Resources/Scripts/Dymola/Applications/AirConditioningSystems/PhotovoltaicCoolingSystem.mos" "Simulate and plot"),
      Diagram(coordinateSystem(preserveAspectRatio = false, extent = {{-60, 60}, {260, -160}}), graphics = {Rectangle(origin = {69, -12}, extent = {{5, 2}, {-5, -2}})}),
      Icon(coordinateSystem(preserveAspectRatio = false, extent = {{-100, -40}, {100, 40}})),
      Documentation(info = "<html>
  <p>
  Example that simulates a photovoltaic driven air-conditioning system:
